...
function approvalKey(chatId, messageId) {
    return `${chatId}:${messageId}`;
}
export function registerApprovalHandlers(bot) {
    bot.on('callback_query:data', async (ctx, next) => {
        ...
        await bot.api.editMessageText(pending.chatId, messageId, approved
            ? '✅ *Approved* — running Codex…'
            : '❌ *Denied* — task cancelled.', { parse_mode: 'Markdown' }).catch(() => undefined);
    });
}
export async function requestApproval(bot, chatId, requesterId, prompt) {
    ...
    const keyboard = new InlineKeyboard()
        .text('✅ Allow', 'approve')
        .text('❌ Deny', 'deny');
    const msg = await bot.api.sendMessage(chatId, `⏳ *Awaiting approval*\n\`\`\`\n${fencedPreview}\n\`\`\`\n_Timeout: ${Math.round(config.approvalTimeoutMs / 60000)} min_`, {
        parse_mode: 'Markdown',
    ...
    return new Promise((resolve) => {
        const timer = setTimeout(async () => {
            approvals.delete(key);
            resolve(false);
            await bot.api.editMessageText(chatId, msg.message_id, '⏱️ *Timed out* — task cancelled.', { parse_mode: 'Markdown' }).catch(() => undefined);
